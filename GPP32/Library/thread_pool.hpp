    ...
    */
    ...
    public:
        ...
        auto push(F&& func_, Args&&... args_) -> std::future<std::invoke_result_t<F, Args...>> {
            ...
            auto task_ptr_ = std::make_shared<std::packaged_task<ResultType()>>([func = std::forward<F>(func_), args_tuple = std::make_tuple(std::forward<Args>(args_)...)]()mutable {
        ...
        auto push_loop(Iterator begin_, Iterator end_, F&& func_) -> std::vector<std::future<std::invoke_result_t<F, typename std::iterator_traits<Iterator>::value_type>>> {
