...
namespace flutter {
    ...
    public:
        ...
        explicit PluginRegistrarWindows(FlutterDesktopPluginRegistrarRef core_registrar) : PluginRegistrar(core_registrar) {
        ...
        ~PluginRegistrarWindows() override {
        ...
        // Prevent copying.
        PluginRegistrarWindows(const PluginRegistrarWindows&) = delete;
        auto operator=(const PluginRegistrarWindows&) -> PluginRegistrarWindows& = delete;
        ...
        auto GetView() -> FlutterView* {
            return implicit_view_.get();
        }
        ...
        auto GetViewById(FlutterViewId view_id) const -> std::shared_ptr<FlutterView> {
            FlutterDesktopViewRef view = FlutterDesktopPluginRegistrarGetViewById(registrar(), view_id);
            if (!view) {
                return nullptr;
            }
        ...
        auto RegisterTopLevelWindowProcDelegate(WindowProcDelegate delegate) -> int {
            if (window_proc_delegates_.empty()) {
                FlutterDesktopPluginRegistrarRegisterTopLevelWindowProcDelegate(registrar(), OnTopLevelWindowProc, this);
            }
        ...
        auto UnregisterTopLevelWindowProcDelegate(int proc_id) -> void {
            window_proc_delegates_.erase(proc_id);
            if (window_proc_delegates_.empty()) {
                FlutterDesktopPluginRegistrarUnregisterTopLevelWindowProcDelegate(registrar(), OnTopLevelWindowProc);
            }
        }
    ...
    private:
        ...
        static auto OnTopLevelWindowProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam, void* user_data, LRESULT* result) -> bool {
            const auto* registrar = static_cast<PluginRegistrarWindows*>(user_data);
            std::optional optional_result = registrar->CallTopLevelWindowProcDelegates(hwnd, message, wparam, lparam);
            if (optional_result) {
        ...
        auto CallTopLevelWindowProcDelegates(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) const -> std::optional<LRESULT> {
