...
namespace flutter {
    ...
    public:
        ...
        explicit FlutterEngine(const DartProject& _project);

        ~FlutterEngine() override;

        // Prevent copying.
        FlutterEngine(const FlutterEngine&) = delete;
        auto operator=(const FlutterEngine&) -> FlutterEngine& = delete;
        ...
        auto Run(const char* entry_point) -> bool;
        ...
        auto ProcessMessages() const -> std::chrono::nanoseconds;
        ...
        // Win32 application).
        auto ReloadSystemFonts() const -> void;
        ...
        auto ReloadPlatformBrightness() -> void;

        // flutter::PluginRegistry:
        auto GetRegistrarForPlugin(const std::string& plugin_name) -> FlutterDesktopPluginRegistrarRef override;
        ...
        auto SetNextFrameCallback(std::function<void()> callback) -> void;
        ...
        auto ProcessExternalWindowMessage(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) const -> std::optional<LRESULT>;
    ...
    private:
        ...
        auto RelinquishEngine() -> FlutterDesktopEngineRef;
