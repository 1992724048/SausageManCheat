...
namespace flutter {
    ...
    public:
        ...
        StreamHandlerFunctions(StreamHandlerListen<T> on_listen, StreamHandlerCancel<T> on_cancel) : on_listen_(on_listen), on_cancel_(on_cancel) {}

        ~StreamHandlerFunctions() override = default;

        // Prevent copying.
        StreamHandlerFunctions(const StreamHandlerFunctions&) = delete;
        auto operator=(const StreamHandlerFunctions&) -> StreamHandlerFunctions& = delete;

    protected:
        // |flutter::StreamHandler|
        auto OnListenInternal(const T* arguments, std::unique_ptr<EventSink<T>>&& events) -> std::unique_ptr<StreamHandlerError<T>> override {
            if (on_listen_) {
                return on_listen_(arguments, std::move(events));
            }

            auto error = std::make_unique<StreamHandlerError<T>>("error", "No OnListen handler set", nullptr);
            return std::move(error);
        }

        // |flutter::StreamHandler|
        auto OnCancelInternal(const T* arguments) -> std::unique_ptr<StreamHandlerError<T>> override {
            if (on_cancel_) {
                return on_cancel_(arguments);
            }

            auto error = std::make_unique<StreamHandlerError<T>>("error", "No OnCancel handler set", nullptr);
            return std::move(error);
        }
