...
namespace flutter {
    ...
    public:
        ...
        MethodResultFunctions(ResultHandlerSuccess<T> on_success, ResultHandlerError<T> on_error, ResultHandlerNotImplemented<T> on_not_implemented) : on_success_(on_success),
            on_error_(on_error),
            on_not_implemented_(std::move(on_not_implemented)) {}

        ~MethodResultFunctions() override = default;

        // Prevent copying.
        MethodResultFunctions(const MethodResultFunctions&) = delete;
        auto operator=(const MethodResultFunctions&) -> MethodResultFunctions& = delete;

    protected:
        // |flutter::MethodResult|
        auto SuccessInternal(const T* result) -> void override {
            if (on_success_) {
                on_success_(result);
            }
        }
