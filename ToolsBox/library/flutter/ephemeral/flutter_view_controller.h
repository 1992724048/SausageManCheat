...
namespace flutter {
    ...
    public:
        ...
        // Prevent copying.
        FlutterViewController(const FlutterViewController&) = delete;
        auto operator=(const FlutterViewController&) -> FlutterViewController& = delete;

        // Returns the view controller's view ID.
        auto view_id() const -> FlutterViewId;

        // Returns the engine running Flutter content in this view.
        auto engine() const -> FlutterEngine* {
            return engine_.get();
        }

        // Returns the view managed by this controller.
        auto view() const -> FlutterView* {
            return view_.get();
        }

        // Requests new frame from the engine and repaints the view.
        auto ForceRedraw() const -> void;
        ...
        auto HandleTopLevelWindowProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) const -> std::optional<LRESULT>;
