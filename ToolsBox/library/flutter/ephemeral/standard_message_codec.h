...
namespace flutter {
    ...
    public:
        ...
        static auto GetInstance(const StandardCodecSerializer* serializer = nullptr) -> const StandardMessageCodec&;

        ~StandardMessageCodec() override;

        // Prevent copying.
        StandardMessageCodec(const StandardMessageCodec&) = delete;
        auto operator=(const StandardMessageCodec&) -> StandardMessageCodec& = delete;

    protected:
        // |flutter::MessageCodec|
        auto DecodeMessageInternal(const uint8_t* binary_message, size_t message_size) const -> std::unique_ptr<EncodableValue> override;

        // |flutter::MessageCodec|
        auto EncodeMessageInternal(const EncodableValue& message) const -> std::unique_ptr<std::vector<uint8_t>> override;
