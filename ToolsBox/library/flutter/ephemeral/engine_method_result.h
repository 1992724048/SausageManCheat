...
namespace flutter {namespace internal {
        ...
        public:
            ...
            // Prevent copying.
            ReplyManager(const ReplyManager&) = delete;
            auto operator=(const ReplyManager&) -> ReplyManager& = delete;
            ...
            // the engine.
            auto SendResponseData(const std::vector<uint8_t>* data) -> void;
    ...
    public:
        ...
        // object exists.
        EngineMethodResult(BinaryReply reply_handler, const MethodCodec<T>* codec) : reply_manager_(std::make_unique<internal::ReplyManager>(std::move(reply_handler))), codec_(codec) {}

        ~EngineMethodResult() override = default;

    protected:
        // |flutter::MethodResult|
        auto SuccessInternal(const T* _result) -> void override {
            const std::unique_ptr<std::vector<uint8_t>> data = codec_->EncodeSuccessEnvelope(_result);
        ...
        auto ErrorInternal(const std::string& error_code, const std::string& error_message, const T* error_details) -> void override {
            const std::unique_ptr<std::vector<uint8_t>> data = codec_->EncodeErrorEnvelope(error_code, error_message, error_details);
            reply_manager_->SendResponseData(data.get());
        }

        // |flutter::MethodResult|
        auto NotImplementedInternal() -> void override {
